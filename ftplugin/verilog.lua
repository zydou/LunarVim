...
-- filetype: verilog
-- server(s): hdl_checker, svlangserver, svls, verible

-- NOTE: multiple servers manage this filetype; all are attempted
local mgr_ok, mgr = pcall(require, "lvim.lsp.manager")
if mgr_ok then
  pcall(function() mgr.setup('hdl_checker') end)
  pcall(function() mgr.setup('svlangserver') end)
  pcall(function() mgr.setup('svls') end)
  pcall(function() mgr.setup('verible') end)
end
