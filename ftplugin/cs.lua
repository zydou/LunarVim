...
-- filetype: cs
-- server(s): csharp_ls, omnisharp, omnisharp_mono

-- NOTE: multiple servers manage this filetype; all are attempted
local mgr_ok, mgr = pcall(require, "lvim.lsp.manager")
if mgr_ok then
  pcall(function() mgr.setup('csharp_ls') end)
  pcall(function() mgr.setup('omnisharp') end)
  pcall(function() mgr.setup('omnisharp_mono') end)
end
