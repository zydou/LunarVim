...
-- filetype: php
-- server(s): intelephense, phpactor, psalm, stimulus_ls, tailwindcss

-- NOTE: multiple servers manage this filetype; all are attempted
local mgr_ok, mgr = pcall(require, "lvim.lsp.manager")
if mgr_ok then
  pcall(function() mgr.setup('intelephense') end)
  pcall(function() mgr.setup('phpactor') end)
  pcall(function() mgr.setup('psalm') end)
