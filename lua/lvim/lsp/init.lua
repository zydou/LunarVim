...
function M.common_on_exit(_, _)
  if lvim.lsp.document_highlight then autocmds.clear_augroup("lsp_document_highlight") end
  if lvim.lsp.code_lens_refresh then autocmds.clear_augroup("lsp_code_lens_refresh") end
end
...
function M.common_on_attach(client, bufnr)
  ...
  if lvim.lsp.document_highlight then lu.setup_document_highlight(client, bufnr) end
  if lvim.lsp.code_lens_refresh then lu.setup_codelens_refresh(client, bufnr) end
...
function M.setup()
  ...
  local function set_handler_opts_if_not_set(name, handler, opts)
    if debug.getinfo(vim.lsp.handlers[name], "S").source:find(vim.env.VIMRUNTIME, 1, true) then vim.lsp.handlers[name] = vim.lsp.with(handler, opts) end
  end
