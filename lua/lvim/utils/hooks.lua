...
function M.run_pre_update() Log:debug("Starting pre-update hook") end
...
function M.run_post_update()
  ...
  if vim.fn.has("nvim-0.9") ~= 1 then
    ...
    vim.notify("Please upgrade your Neovim base installation. Newer version of Lunarvim requires v0.9+", vim.log.levels.WARN)
    vim.wait(1000)
    local ret = reload("lvim.utils.git").switch_lvim_branch(compat_tag)
    if ret then vim.notify("Reverted to the last known compatible version: " .. compat_tag, vim.log.levels.WARN) end
    return
  end
  ...
  if not in_headless then
    vim.schedule(function()
      if package.loaded["nvim-treesitter"] then vim.cmd([[ TSUpdateSync ]]) end
