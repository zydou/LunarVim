...
local function _assign(old, new, k)
  ...
  -- print("hi")
  if (otype == "thread" or otype == "userdata") or (ntype == "thread" or ntype == "userdata") then vim.notify(string.format("warning: old or new attr %s type be thread or userdata", k)) end
  old[k] = new[k]
end
  ...
  for k, _ in pairs(new) do
    if not old[k] then
      ...
      if type(old[k]) ~= type(new[k]) then
        Log:debug(string.format("Reloader: mismatch between old [%s] and new [%s] type for [%s]", type(old[k]), type(new[k]), k))
        _assign(old, new, k)
      else
...
M.reload = function(mod)
  if not package.loaded[mod] then return M.require_safe(mod) end
