...
local function deprecate(name, alternative)
  ...
  vim.schedule(function() vim.notify_once(string.format("%s: `%s` is deprecated.\n %s.", t, name, alternative), vim.log.levels.WARN) end)
end

function M.handle()
  local mt = {
    __newindex = function(_, k, _) deprecate(k) end,
  }
  ...
  setmetatable(lvim.builtin.notify, {
    __newindex = function(_, k, _) deprecate("lvim.builtin.notify." .. k, "See LunarVim#3294") end,
  })

  ---@deprecated
  lvim.builtin.dashboard = {}
  setmetatable(lvim.builtin.dashboard, {
    __newindex = function(_, k, _) deprecate("lvim.builtin.dashboard." .. k, "Use `lvim.builtin.alpha` instead. See LunarVim#1906") end,
  })
  ...
  setmetatable(lvim.lsp.float, {
    __newindex = function(_, k, _) deprecate("lvim.lsp.float." .. k, "Use options provided by the handler instead") end,
  })
...
function M.post_load()
  if lvim.lsp.diagnostics and not vim.tbl_isempty(lvim.lsp.diagnostics) then vim.diagnostic.config(lvim.lsp.diagnostics) end
  ...
  if lvim.lsp.override and not vim.tbl_isempty(lvim.lsp.override) then
    ...
    vim.tbl_map(function(c)
      if not vim.tbl_contains(lvim.lsp.automatic_configuration.skipped_servers, c) then table.insert(lvim.lsp.automatic_configuration.skipped_servers, c) end
    end, lvim.lsp.override)
  end

  if lvim.autocommands.custom_groups then deprecate("lvim.autocommands.custom_groups", "Use vim.api.nvim_create_autocmd instead or check LunarVim#2592 to learn about the new syntax") end

  if lvim.lsp.automatic_servers_installation then deprecate("lvim.lsp.automatic_servers_installation", "Use `lvim.lsp.installer.setup.automatic_installation` instead") end
  ...
  local function convert_spec_to_lazy(spec)
    ...
    alternatives.disable = function()
      if type(spec.disabled) == "function" then
        spec.enabled = function() return not spec.disabled() end
      else
    ...
    alternatives.wants = function() return "dependencies = [value]" end
...
See https://github.com/folke/lazy.nvim#-migration-guide"]],
  ...
  for _, plugin in ipairs(lvim.plugins) do
    if type(plugin) == "table" then convert_spec_to_lazy(plugin) end
  end
end
