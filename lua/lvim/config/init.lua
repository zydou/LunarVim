...
---@return string
function M:get_user_config_path() return user_config_file end
...
function M:load(config_path)
  ...
  if not ok then
    if utils.is_file(user_config_file) then
      vim.schedule(function() Log:warn("Invalid configuration: " .. err) end)
    else
      vim.schedule(function() vim.notify_once(string.format("User-configuration not found. Creating an example configuration in %s", config_path)) end)
  ...
  if lvim.transparent_window then autocmds.enable_transparent_mode() end

  if lvim.reload_config_on_save then autocmds.enable_reload_config_on_save() end
end
