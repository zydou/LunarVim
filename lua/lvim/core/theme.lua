...
M.setup = function()
  ...
  if vim.startswith(lvim.colorscheme, selected_theme) then
    ...
    pcall(function() plugin.setup(lvim.builtin.theme[selected_theme].options) end)
  end
  ...
  if package.loaded.lualine then require("lvim.core.lualine").setup() end
  if package.loaded.lir then require("lvim.core.lir").icon_setup() end
end
