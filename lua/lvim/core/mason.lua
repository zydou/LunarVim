...
local function add_to_path(append)
  ...
  if vim.env.PATH:match(p) then return end
...
function M.bootstrap() add_to_path() end
...
function M.setup()
  ...
  if lvim.builtin.mason.on_config_done then lvim.builtin.mason.on_config_done(mason) end
end
