...
local function str_list(list) return #list == 1 and list[1] or fmt("[%s]", table.concat(list, ", ")) end
...
local function make_formatters_info(ft)
  ...
  local section = {
    "Formatters info",
    fmt("* Active: %s%s", table.concat(registered_formatters, " " .. lvim.icons.ui.BoxChecked .. " , "), vim.tbl_count(registered_formatters) > 0 and " " .. lvim.icons.ui.BoxChecked .. " " or ""),
...
local function make_code_actions_info(ft)
  ...
  local section = {
    "Code actions info",
    fmt("* Active: %s%s", table.concat(registered_actions, " " .. lvim.icons.ui.BoxChecked .. " , "), vim.tbl_count(registered_actions) > 0 and " " .. lvim.icons.ui.BoxChecked .. " " or ""),
  }
...
local function make_linters_info(ft)
  ...
  local section = {
    "Linters info",
    fmt("* Active: %s%s", table.concat(registered_linters, " " .. lvim.icons.ui.BoxChecked .. " , "), vim.tbl_count(registered_linters) > 0 and " " .. lvim.icons.ui.BoxChecked .. " " or ""),
...
local function make_auto_lsp_info(ft)
  ...
  local skipped = vim.tbl_filter(function(name) return vim.tbl_contains(supported, name) end, skipped_servers)
...
function M.toggle_popup(ft)
  ...
  local is_treesitter_active = function()
    ...
    if vim.tbl_contains(ts_active_buffers, bufnr) then status = "active" end
    return status
  end
  local header = {
    "Buffer info",
  ...
  for _, client in pairs(clients) do
    ...
    if client_info then vim.list_extend(lsp_info, client_info) end
