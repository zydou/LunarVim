...
function Log:init()
  ...
  local log_level = Log.levels[(lvim.log.level):upper() or "DEBUG"]
  structlog.configure({
    lvim = {
      pipelines = {
        {
...
function Log:configure_notifications(nvim_notify)
  ...
  local function log_writer(log)
    ...
    if log_notify_as_notification then nvim_notify(log.msg, log.level, opts) end
  end

  local notif_pipeline = structlog.Pipeline(structlog.level.INFO, {}, structlog.formatters.Format("%s", { "msg" }, { blacklist_all = true }), structlog.sinks.Adapter(log_writer))
